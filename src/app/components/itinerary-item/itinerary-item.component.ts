...
export class ItineraryItemComponent {
  ...
  // CREATE
  addItineraryItem(){
    this.router.navigate([`../trips/${this.selectedCorrelatedTrip?.trip._id}/itinerary-items/add`], { relativeTo: this.route })
  }

  // UPDATE
  editItineraryItem(itineraryItem: ItineraryItem){
    this.router.navigate([`..../trips/${this.selectedCorrelatedTrip?.trip._id}/itinerary-items/${itineraryItem._id}/edit`], { relativeTo: this.route })
  }
  // DELETE
